...
if __name__ == "__main__":
    if len(sys.argv) == 3:
            net, mean, std = train(sys.argv[1], sys.argv[2])
            dict = {'net': net, 'mean': mean, 'std': std}
            fileObject = open("modellasagne.dat", 'w')
            pickle.dump(dict, fileObject)
            fileObject.close()
    else:
        print("The script needs 2 arguments : \n1: Train file \n2: Weather csv file \n"
              "Example: python train.py ./input/train.csv ./input/weather.csv")
